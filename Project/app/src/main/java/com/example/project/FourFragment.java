...
public class FourFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btn_rating = (Button) v.findViewById(R.id.btn_rating4);
        btn_date = (Button) v.findViewById(R.id.btn_date4);
        btn_reset = (Button) v.findViewById(R.id.btn_reset4);

        //배경 이미지 투명도 50 적용
        Drawable alpha = ((ImageView) v.findViewById(R.id.imageView6)).getDrawable();
        alpha.setAlpha(50);

        //메인 컨테이너 > 인텐트 컨테이너 변경
        mainContainer = (LinearLayout) v.findViewById(R.id.main_input4);

        //DB 읽어오기
        Cursor cursor = myBistroDB.rawQuery("select rating, type, title, location, strftime('%Y-%m-%d', visited), review from myRestaurant where type='일식' order by rating desc, visited asc", null);
